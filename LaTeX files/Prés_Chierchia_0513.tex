...
\subsection{Scalar Implicatures}
...
\begin{description}
  \item[Maximize Strength:] Don't add an implicature if it leads to weakening, unless you are forced to by the context.
\end{description}
...
\subsection{Extended Standard Grice Theory and properties of SIs}
The \emph{ESG} theory proposed by Chierchia can be stated as follows:
...
\exs[]{
  ...
  \ex \begin{tabular}[t]{llp{13cm}}\toprule[-12.5pt]
  		ALT & $=$ & $\{\neg\exists{x \in D^\prime}[$cookies$(x) \wedge $left$(x)]: D^\prime \subseteq D]$\}\\
